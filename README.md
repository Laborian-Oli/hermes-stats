...
- **成本一律按人民币（¥）展示**：统计管线里的估价是美元口径（模型单价来自官方定价），后端按 USD→CNY 汇率折算后下发
  - 汇率来源：ECB 参考汇率（frankfurter.dev）→ exchangerate-api 兜底；缓存 12h，接口不可达时沿用旧值并标注"缓存值"
  - 页面「会话与成本」标题旁的 `FX 1 USD = ¥x.xxxx` 标签即为本次折算所用汇率；汇率源不可达且无缓存时退回内置常数 7.10
  - 银行结汇/购汇价（现汇卖出价）与 ECB 参考汇率仍有价差，本插件按参考汇率展示，仅供量级参考
